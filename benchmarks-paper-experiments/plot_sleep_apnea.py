...
x = np.linspace(0, 34000, 34000)
data = sleep.get_inputs(TStart, TStart+TTotal, normalise=False)
...
for i in range(3):
    axen[i].plot(x[TStart:TStart+TTotal], data[i], color='green')
...
axen[0].set_title("heart rate", fontsize=24)
axen[1].set_title("respiration", fontsize=24)

axen[2].set_title("blood oxygenation", fontsize=24)
