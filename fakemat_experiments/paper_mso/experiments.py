...
def experiment_single_timescale(MSO, name, normalise_svd=False, save=False, debug=False):
    print("start")
    nrmsedf = pd.DataFrame(columns=['bucket', 'delayline', 'maglattice'])
    ...
    if save:
        if normalise_svd:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_mso/results/normalised_svd/single_timescale_{name}.csv"
        else:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_mso/results/single_timescale_{name}.csv"
...
def experiment_multi_timescale(MSO, name, normalise_svd=False, save=False):
    ...
    if save:
        if normalise_svd:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_mso/results/normalised_svd/multi_timescale_{name}.csv"
        else:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_mso/results/multi_timescale_{name}.csv"
        nrmsedf.to_csv(buf_nrmse)
    print("done!")
    return
