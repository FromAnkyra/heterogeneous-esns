...
def experiment_single_timescale(MSO, name, normalise_svd=False, save=False, debug=False):
    print("start")
    nrmsedf = pd.DataFrame(columns=['bucket', 'delayline', 'sdelayline', 'maglattice', 'mixed', 'mixed-symmetrical'])
    for i in range(TRuns):
        ...
        sdelaylineesn = create_esns.create_esn_single_timescale(create_esns.sdelayline_matlist, fullsize, i, normalise_svd)
        sdelaylinenrmse, _ = mso.run_MSO_rr(sdelaylineesn, MSO, data_lengths, error="both")
        nrmsedf.at[i, 'sdelayline'] = sdelaylinenrmse[1]

        mixedesn = create_esns.create_esn_single_timescale(create_esns.mixed_matlist, fullsize, i, normalise_svd)
        mixednrmse, _ = mso.run_MSO_rr(mixedesn, MSO, data_lengths, error="both")
        nrmsedf.at[i, 'mixed'] = mixednrmse[1]

        smixedesn = create_esns.create_esn_single_timescale(create_esns.mixed_symmetrical_matlist, fullsize, i, normalise_svd)
        smixednrmse, _ = mso.run_MSO_rr(smixedesn, MSO, data_lengths, error="both")
        nrmsedf.at[i, "mixed-symmetrical"] = smixednrmse[1]

    if save:
        if normalise_svd:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_spectral_radius/results/normalised_sr_{name}.csv"
        else:
            buf_nrmse = f"/home/cw1647/phd/fakemat_experiments/paper_spectral_radius/results/standard_{name}.csv"
...
experiment_single_timescale(MSO_eight, "mso_eight", normalise_svd=True, save=True)

print("without scaling")
experiment_single_timescale(MSO_two, "mso_two", normalise_svd=False, save=True)
experiment_single_timescale(MSO_four, "mso_four", normalise_svd=False, save=True)
experiment_single_timescale(MSO_eight, "mso_eight", normalise_svd=False, save=True)
