...
density = 0.2
N = 50  # no of nodes
NARMA = 20  # which NARMA function to use
...
config = ['flat'] # only use flat config
config_name = [''] 
...
for seed in range(Nruns):
    print(seed)
    rseed = 1    # option to have a different seed for reservoir setup, input setup
    inseed = seed
    #np.random.seed(seed)
    ...
    while system.loc[Ttot - 1, 'y'] == np.inf:
        ...
        system['u'] = get_u(Ttot, seed=inseed)
        # get the NARMA states and outputs
        run_narma(NARMA, Ttot, system)

    # reservoir random weights
    rc = reservoir.Reservoir(N, rho=rho, density=density, seed=rseed)
...
reservoir.plot_results(system, errordf, Twashout, Ttrain, Ttest, 
        'narma'+str(NARMA)+'-wash'+str(Twashout)+'-train'+str(Ttrain)+'-test'+str(Ttest)+'-runs'+str(Nruns)+'-r'+str(rseed)+'.pdf', boxplot_only=True)
