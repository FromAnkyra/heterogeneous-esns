...
def compare_distributions(N, n_subgroups):
    ...
    for i in range(TRuns):
        ...
        snrmse, smse = mso.run_MSO(standard, MSO_three, data_lengths, error="both")
        rnrmse, rmse = mso.run_MSO(restricted, MSO_three, data_lengths, error="both")
        # print(f"{restricted.Wv=}")
        gnrmse, gmse = mso.run_MSO(gondor, MSO_three, data_lengths, error="both")
        # print(f"{gondor.Wv=}")
        cnrmse, cmse = mso.run_MSO(circuit, MSO_three, data_lengths, error="both")
    ...
    # save nrmses
    buf_nrmse = f"/home/cw1647/phd/tempESN-experiments/results/nrmses/mso_three.csv"
    nrmsedf.to_csv(buf_nrmse)

    # save mses 
    buf_mse = f"/home/cw1647/phd/tempESN-experiments/results/mses/mso_three.csv"
