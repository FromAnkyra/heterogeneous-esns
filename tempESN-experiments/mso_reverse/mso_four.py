...
MSO_four = mso.generate_MSO(resolution_4, [mso.MSO.eight.value, mso.MSO.seven.value, mso.MSO.six.value, mso.MSO.seven.value])
MSO_four = MSO_four/8 # scale the input to be between [-0.5, 0.5]
...
def single_density(density):
    ...
    for i in range(TRuns):
        ...
        train, test = mso.run_MSO(ESN, MSO_four, data_lengths)
...
def compare_distributions(N, n_subgroups):
    ...
    rhythm_four = np.array([[1], [0, 1], [0, 0, 1], [0, 0, 0, 1]], dtype=object)
    for i in range(TRuns):
        ...
        snrmse, smse = mso.run_MSO_rr(standard, MSO_four, data_lengths, error="both")
        rnrmse, rmse = mso.run_MSO_rr(restricted, MSO_four, data_lengths, error="both")
        # print(f"{restricted.Wv=}")
        gnrmse, gmse = mso.run_MSO_rr(gondor, MSO_four, data_lengths, error="both")
        # print(f"{gondor.Wv=}")
        cnrmse, cmse = mso.run_MSO_rr(circuit, MSO_four, data_lengths, error="both")
    ...
    # save nrmses
    buf_nrmse = f"/home/cw1647/phd/tempESN-experiments/mso_reverse/results/nrmses/mso_four.csv"
    nrmsedf.to_csv(buf_nrmse)

    # save mses 
    buf_mse = f"/home/cw1647/phd/tempESN-experiments/mso_reverse/results/mses/mso_four.csv"
